...
DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "grain_monitor.db")
if not os.path.exists(DB):
    import build_db; build_db.build()
else:
    # Apply any schema migrations to existing database (safe to run every startup)
    import build_db; build_db.migrate(sqlite3.connect(DB))
...
with tab6:
        ...
        if st.button("💾 Save Analysis"):
            run("""INSERT INTO s_tarawa_analysis
                ...
                ON CONFLICT(report_month, commodity) DO UPDATE SET
                    ...
                    est_days=excluded.est_days, last_date=excluded.last_date,
    ...
    # ── Supplier ──────────────────────────────────────────────────────────────
        ...
        # ── Migrate: ensure extra columns exist on old DBs ───────────────────
        for col_def in [
        ...
        sup_mode = st.radio("Action", ["➕ Add New Supplier", "✏️ Edit / Update Supplier"],
                            horizontal=True)
        st.markdown("---")
            ...
            if st.button("💾 Save New Supplier"):
                if not sup_name.strip():
                    ...
                    try:
                        ...
                        st.success(f"✅ Supplier '{sup_name.strip()}' added successfully.")
                        st.cache_data.clear(); st.rerun()
        ...
        # ════════════════════════════════════════════════════════════════════
        # EDIT / UPDATE / DELETE EXISTING SUPPLIER
        # ════════════════════════════════════════════════════════════════════
        else:
            ...
            if existing_sups.empty:
                st.info("No suppliers on record yet. Add one using '➕ Add New Supplier'.")
            else:
                sup_names = existing_sups["name"].tolist()
                selected_name = st.selectbox("Select Supplier to Edit", sup_names)
                ...
                c1, c2 = st.columns(2)
                edit_name    = c1.text_input("Supplier Name *",
                                             value=sel_row["name"] or "")
                edit_contact = c2.text_input("Contact Person",
                                             value=sel_row["contact_person"] or "")

                c3, c4 = st.columns(2)
                edit_phone = c3.text_input("Phone",
                                           value=sel_row["phone"] or "")
                edit_email = c4.text_input("Email",
                                           value=sel_row["email"] or "")

                edit_address = st.text_input("Address",
                                             value=sel_row["address"] or "")
                edit_notes   = st.text_area("Notes",
                                            value=sel_row["notes"] or "",
                                            height=80)
                ...
                with col_save:
                    if st.button("💾 Update Supplier"):
                        if not edit_name.strip():
                            ...
                            try:
                                run("""UPDATE suppliers
                                    SET name=?, contact_person=?, phone=?, email=?, address=?, notes=?
                                    WHERE id=?""",
                                    (edit_name.strip(), edit_contact.strip(), edit_phone.strip(),
                                     edit_email.strip(), edit_address.strip(), edit_notes.strip(),
                                     sel_id))
                                st.success(f"✅ Supplier '{edit_name.strip()}' updated successfully.")
                                st.cache_data.clear(); st.rerun()
                            except Exception as e:
                                if "UNIQUE constraint" in str(e):
                                    st.error(f"⚠️ Another supplier named '{edit_name.strip()}' already exists.")
                                else:
                                    st.error(f"❌ Error updating supplier: {e}")

                with col_del:
                    # Two-step delete: confirm before executing
                ...
                if st.session_state.get("confirm_delete_sup") == sel_id:
                    st.warning(
                        f"⚠️ Are you sure you want to delete **{selected_name}**? "
                        "This cannot be undone. Cargo arrival records linked to this supplier will be affected."
                    )
                    cc1, cc2 = st.columns(2)
                    if cc1.button("✅ Yes, Delete"):
                        run("DELETE FROM suppliers WHERE id=?", (sel_id,))
                        st.session_state.pop("confirm_delete_sup", None)
                        st.success(f"🗑️ Supplier '{selected_name}' deleted.")
                        st.cache_data.clear(); st.rerun()
                    if cc2.button("❌ Cancel"):
                        st.session_state.pop("confirm_delete_sup", None)
                        st.rerun()

        # ── Existing Suppliers table (always visible) ────────────────────────
        st.markdown("---")
        ...
        if all_sups.empty:
            ...
            st.download_button("⬇️ Download CSV", all_sups.to_csv(index=False),
