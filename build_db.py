...
def migrate(con):
    ...
    for col_def in [
        ...
        try:
            con.execute(f"ALTER TABLE suppliers ADD COLUMN {col_def}")
        except Exception:
            pass
    ...
    if not any("unique" in i.lower() or "sta_unique" in i.lower() for i in indexes):
        try:
            con.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_sta_unique "
                "ON s_tarawa_analysis(report_month, commodity)"
            )
        except Exception:
            pass
